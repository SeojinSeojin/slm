#!/usr/bin/env python3
# run_ours_d_only.py
# Method D: Answer-Weighted Loss
# Train on all tokens (like CT) but apply higher weight to answer tokens (after ####).
...
CONFIG = {
    ...
    "asdiv_samples":     200,
    "svamp_samples":     200,
    "answer_weight":     3.0,   # weight applied to answer tokens (after ####)
}
...
# D uses Ours-B scored file (symbolic masking ~50%) + answer weighting
scored_path = os.path.join(DATA_DIR, "gsm8k_symbolic_span_w1.jsonl")

if not os.path.exists(scored_path):
    print(f"ERROR: {scored_path} not found.")
    print("Run Ours-B scoring first: python3 run_scoring_only.py")
    exit(1)

exp_key    = "ours_d_answer_weight"
exp_name   = "Ours-D"
...
if already_evaluated(exp_name):
    ...
    print(f"Training: {exp_name}  (answer_weight={CONFIG['answer_weight']})")
    clear_memory()
    dataset = SLMDataset(
        scored_path,
        CONFIG["max_length"],
