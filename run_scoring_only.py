...
if RUN_RHO1 and need_scoring(rho1_path):
    ...
    preprocess_dataset_rho1(data_path=RAW_PATH, output_path=rho1_path,
...
if RUN_OURS_B and need_scoring(b_path):
    ...
    preprocess_dataset_symbolic(data_path=RAW_PATH, output_path=b_path,
...
if RUN_OURS_A and need_scoring(a_path):
    print("\n[Ours-A] Entropy gap scoring...")
    clear_memory()
    from symbolic_slm.scoring.entropy_gap_scorer import preprocess_dataset_entropy_gap
    preprocess_dataset_entropy_gap(data_path=RAW_PATH, output_path=a_path,
                                    select_ratio=0.25, max_length=MAX_LENGTH)
    clear_memory()
    print("[Ours-A] Done.")
...
if RUN_OURS_C and need_scoring(c_path):
    ...
    preprocess_dataset_math_confidence(data_path=RAW_PATH, output_path=c_path,
                                        gap_ratio=0.40, abs_ratio=0.40,
                                        context_window=OURS_C_CONTEXT_WIN,
                                        max_length=MAX_LENGTH)
    clear_memory()
    print("[Ours-C] Done.")
...
for label, path in [("RHO-1", rho1_path), ("Ours-B", b_path),
                     ("Ours-A", a_path), ("Ours-C", c_path)]:
...
print("=" * 55)
print("All scoring complete. Ready for training.")
